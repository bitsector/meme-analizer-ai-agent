...
class AuthConfig:
    ...
    def __init__(self):
        try:
            ...
            logger.info(
                f"Client Secret (length): {len(self.google_client_secret)} chars"
            )
...
class AuthService:
    ...
    def get_google_auth_url() -> str:
        ...
        try:
            logger.info(
                f"Creating OAuth client with client_id: {auth_config.google_client_id[:20]}..."
            )
            ...
            authorization_url, state = oauth_client.create_authorization_url(
                auth_config.google_auth_url, scope=["openid", "email", "profile"]
            )
    ...
    async def exchange_code_for_token(authorization_code: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting token exchange for authorization code: {authorization_code[:10]}..."
        )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                token_response = await client.post(
                    auth_config.google_token_url, data=token_data
                )
            ...
            async with httpx.AsyncClient() as client:
                ...
                user_response = await client.get(
                    auth_config.google_userinfo_url, headers=headers
                )
