...
def get_secret(secret_name: str) -> str:
    ...
    if env_value:
        logger.debug(
            f"Reading secret {secret_name} from environment variable (original case)"
        )
        return env_value

    raise ValueError(
        f"Secret {secret_name} not found in file (/run/secrets/{secret_name}) or environment"
    )
    ...
    def __init__(self):
        ...
        if self.llm_provider == "openai":
            try:
                ...
                if not self.api_key:
                    raise ValueError(
                        "OpenAI API key not found in secrets or environment variables"
                    )
            ...
            try:
                ...
                if not self.api_key:
                    raise ValueError(
                        "Gemini API key not found in secrets or environment variables"
                    )
        else:
        ...
        if self.llm_provider == "openai":
            self.completion_model = os.getenv(
                "OPENAI_COMPLETION_MODEL", os.getenv("COMPLETION_MODEL", "gpt-4o-mini")
            )
            self.image_gen_model = os.getenv(
                "OPENAI_IMAGE_GEN_MODEL", os.getenv("IMAGE_GEN_MODEL", "dall-e-2")
            )
            self.tts_model = os.getenv(
                "OPENAI_TTS_MODEL", os.getenv("TTS_MODEL", "tts-1")
            )
        elif self.llm_provider == "gemini":
            self.completion_model = os.getenv(
                "GEMINI_COMPLETION_MODEL",
                os.getenv("COMPLETION_MODEL", "gemini-2.5-flash-lite"),
            )
...
def print_detailed_graph_structure():
    ...
    try:
        ...
        logger.info(
            "  • search: Searches web for ARTICLE/FACTS content using DuckDuckGo"
        )
        logger.info(
            "  • meme_name_analysis: Identifies meme format names for MEME content"
        )
        logger.info("  • explain_humor_analysis: Explains why memes are funny")
        logger.info(
            "  • social_media_detection: Identifies social media platform from UI"
        )
        logger.info(
            "  • recognise_poster: Identifies who posted the social media content"
        )
        logger.info(
        ...
        logger.info(
            "  • outrage_analysis: Detects outrage/inflammatory content (YES/NO)"
        )
        ...
        logger.info(
            "    ├─ if MEME → meme_name_analysis → explain_humor_analysis → sentiment_analysis"
        )
        logger.info("    ├─ if ARTICLE/FACTS → search → sentiment_analysis")
        logger.info(
            "    ├─ if SOCIAL_MEDIA → social_media_detection → recognise_poster → sentiment_analysis"
        )
        logger.info("    └─ if OTHER → sentiment_analysis")
        logger.info(
            "  sentiment_analysis → political_analysis → outrage_analysis → result → END"
        )
        ...
        logger.info(
            "  • social_media_platform: str - Detected platform (TWITTER/FACEBOOK/etc)"
        )
...
def try_generate_visual_graph(graph):
    ...
    try:
        ...
        logger.warning(
            f"Visual graph generation requires additional packages: {str(ie)}"
        )
